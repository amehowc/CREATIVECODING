const createUniforms = (shader, config) => {
  ...
  class SwapableTexture {
    constructor(target) {
      this.src = new p5.Framebuffer(target, { format: "float" });
      this.dst = new p5.Framebuffer(target, { format: "float" });
    }
    swap() {
      var tmp = this.src;
      this.src = this.dst;
  ...
  function setRenderer(gl, options) {
    if (gl && options) {
      ...
      if (options.extensions) {
        if (options.extensions.lazyload && 
            options.extensions.lazyload === true) {
